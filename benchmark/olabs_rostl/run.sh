...
commit_hash="$(git -C $base_dir/build/${project_name}/rostl/ rev-parse HEAD)"
timestamp="$(date +%s)"
...
cargo run --profile=maxperf 2>&1 | stdbuf -oL tee "$base_dir/logs/$run_id/rostl_bench.log"

# Parse the results
echo "" > "$results_file"
python "$base_dir/scripts/parse.py" -f "$base_dir/logs/$run_id/rostl_bench.log" >> "$results_file"
