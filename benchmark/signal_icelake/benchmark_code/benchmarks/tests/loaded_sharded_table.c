...
int test_loaded_sharded_table(size_t N, size_t batch_size)
{
  ...
  REPORT_LINE("UnorderedMap", "Signal_Sharded", "N:=%zu | Batch_size:=%zu | Key_bytes := 8 | Value_bytes := 56 | Shards:= %d | fill:=0.8 | Initialization_time_us := %.2f | Get_latency_us := %.2f | Get_throughput_qps := %.2f | Memory_kb := %d", N, batch_size, NUM_SHARDS, (end_ns_create - start_ns_create) / 1000.0, avg_ns_success / 1000.0, 1000000000.0 / avg_ns_success, memAfter - memBefore);
