...
for i, implementation in enumerate(['Signal_Sharded', 'olabs_rostl_sharded', 'olabs_oram_sharded']):
  for key_bytes, val_bytes in [(32, 32), (8, 56)]:
    ...
    draw_figure(w1, 'N', 'Get_latency_us',
      f"UMAP - Batch Get Latency vs batch size ({key_bytes}b key, {val_bytes}b value) {i}",
      'name',
      ylabel='$\\mu s$',
      **common_args
    )
    ...
    draw_figure(w1, 'N', 'Get_throughput_qps',
      f"UMAP - Batch Get Throughput vs batch size ({key_bytes}b key, {val_bytes}b value) {i}",
      'name',
      ylabel='Queries per second',
      **common_args
    )
...
for key_bytes, val_bytes in [(32,32), (8, 56)]:
  ...
  draw_figure(w1, 'N', 'Percentage_batch_lb',
    f"UMAP - Batch Breakdown - Load Balancing ({key_bytes}b key, {val_bytes}b value)",
    "Batch_size",
  ...
  draw_figure(w1, 'N', 'Percentage_batch_offline',
    f"UMAP - Batch Breakdown - Offline ({key_bytes}b key, {val_bytes}b value)",
    "Batch_size",
  ...
  draw_figure(w1, 'N', 'Percentage_batch_online',
    f"UMAP - Batch Breakdown - Online  ({key_bytes}b key, {val_bytes}b value)",
    "Batch_size",
